  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div
        ...
        :key="address.id"
      >
        <span class="username" :class="{selected:selectedAddr===address}">{{address.consignee}}</span>
        <p @click="selectedAddr=address">
...
export default {
  ...
  data() {
    return {
      selectedAddr: {}, // 用来存储选中后的地址对象信息(地址/名字/电话)
      orderComment: '老板快点发货啊', // 订单的备注信息
    }
  },
  computed: {
    ...mapState({
  ...
  watch: {
    ...
    'tradeInfo.userAddressList': function (value) {
      // value----->userAddressList数组
      // 如果当前的数组中的某个对象中的isDefault属性值是不是1,如果是1,就证明该对象存在,既然存在直接把对象存储到selectedAddr
  ...
  methods: {
    ...
    async submitOrder() {
      ...
      const orderInfo = {
        ...
        orderDetailList: detailArrayList, // 该订单中的商品数组数据
      }
      // 调用api接口,提交订单操作-----并没有封装到vuex中,是直接调用的
      const result = await this.$API.reqSubmitOrder(tradeNo, orderInfo)
      if (result.code === 200) {
        const orderId = result.data // 当前这个订单的id
        // 跳转的操作,同时还要携带参数
        this.$router.push({ path: '/pay', query: { orderId } })
      } else {
        alert(result.message || '订单提交失败了')
      }
    },
  },
}
